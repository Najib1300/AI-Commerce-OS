...
export function createResearchUsageRecord(context:{organizationId:string;userId:string;runId:string},generation:ResearchGeneration){return {
  ...
  metadata:{model:generation.model,input_tokens:generation.usage.inputTokens,output_tokens:generation.usage.outputTokens,total_tokens:generation.usage.totalTokens,research_run_id:context.runId},
}}
