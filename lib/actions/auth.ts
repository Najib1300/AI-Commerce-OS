...
import { provisionInitialWorkspace } from "@/lib/onboarding";
...
export async function completeOnboarding(formData:FormData){const parsed=onboardingSchema.safeParse(values(formData));if(!parsed.success)fail("/onboarding",parsed.error.issues[0]?.message??"Invalid details");const supabase=await createClient();const result=await provisionInitialWorkspace(supabase,parsed.data);if(result.status==="unauthenticated")redirect("/login");if(result.status==="failed"){console.error("Initial workspace provisioning failed",{detail:result.detail});fail("/onboarding","We couldn’t finish setting up your workspace. Please try again.");}redirect("/dashboard");}
